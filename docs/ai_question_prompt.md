...
> 更新记录：2026-08-04 v11 —— 合并至 t_ai_270，题库量/编号同步。
...
难度分三档，与年级对应：easy→L1（初一上）、medium→L2（初一下）、hard→L3（初三拔高）。

**难度判定标准（重要，出题必须按此标注，禁止全部标 hard）**：
- easy：单句、基础时态（一般现在/过去/将来/进行）、基础情态、简单 There be、简单祈使句
- medium：现在完成时（for/since/just/yet/ever）、过去进行时（when/while/at this time）、if/unless/although/because 简单从句、比较级/最高级、be going to、will、短语动词（look up/look after/put off/give up）、简单被动（一般现在/过去/将来时、情态动词被动）、简单反意疑问句、简单 it 形式主语/宾语
- hard：宾语从句、定语从句、主语/表语从句、过去完成时、间接引语、现在完成时/进行时被动、复合长句（多从句组合）

## 孩子画像（上下文）

- 初一（Grade 7），练习难度已设为 hard（L3 拔高），连续打卡 42 天
- 当前翻译题库 **534 道**（easy 81 / medium 302 / hard 151）
- 当前时态题库 **324 道**（easy 52 / medium 72 / hard 200）
- 已占用 id 前缀：`t_simple` / `t_hard` / `t_llm` / `t_llm2` / `t_curr` / `t_wrong` / `t_ai`
- **AI 题已入库 t_ai_001 ~ t_ai_270**（162 道英译中 + 108 道中译英），新题 id 从 **t_ai_271** 开始连续编号；**禁止从已占用编号（如 031/056/076/101/126/151/176/201）重新开始**
...
- 数量：____ 道（**说好几道就必须输出几道，id 从 t_ai_271 起连续编号，不得缺号；禁止回卷到已占用编号**）
- 难度：按上述标准逐题标注（可混合 easy/medium/hard），**不得全部标 hard**
- 侧重知识点（可指定，默认按孩子错题画像）
...
- 地点介词 10 道
- 介词综合 11 道
- 介词专项巩固 128 道（63 道 AI 扩充 src=ai_expand_2026_08；51 道 Codex 直接补充 src=codex_expand_2026_08）
