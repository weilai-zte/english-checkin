...
|------|-------------|------|
| `functional_requirements` | 本文 §3 (F-001~F-018, 18 条) | json |
...
| **F-018** | P1 | 每周日 20:00 推送本周错词 TOP 10 汇总 + LLM 老师鼓励/分析（按 attempts 降序、已掌握词过滤） | send_weekly_wrong_words.sh cron 触发；DRY_RUN 输出卡片 JSON；时间窗 = today - 7 天 | P2 | 知识课程页面文字对比度提升：正文 #1a1a2e + 标题 #2d3748 + 暗色模式覆盖 | /knowledge 文字清晰可读 |
