...
> 更新记录：2026-08-04 v6 —— 合并至 t_ai_170，题库量/编号同步。
...
- 当前翻译题库 **434 道**（easy 80 / medium 79 / hard 275）
- 已占用 id 前缀：`t_simple` / `t_hard` / `t_llm` / `t_llm2` / `t_curr` / `t_wrong` / `t_ai`
- **AI 题已入库 t_ai_001 ~ t_ai_170**（62 道英译中 + 108 道中译英），新题 id 从 **t_ai_171** 开始连续编号；**禁止从已占用编号（如 031/056/076/101）重新开始**
...
- 数量：____ 道（**说好几道就必须输出几道，id 从 t_ai_171 起连续编号，不得缺号；禁止回卷到已占用编号**）
