...
> 更新记录：2026-08-04 v9 —— 合并至 t_ai_220，题库量/编号同步。
...
- 当前翻译题库 **484 道**（easy 81 / medium 262 / hard 141）
- 已占用 id 前缀：`t_simple` / `t_hard` / `t_llm` / `t_llm2` / `t_curr` / `t_wrong` / `t_ai`
- **AI 题已入库 t_ai_001 ~ t_ai_220**（112 道英译中 + 108 道中译英），新题 id 从 **t_ai_221** 开始连续编号；**禁止从已占用编号（如 031/056/076/101/126/151）重新开始**
...
- 数量：____ 道（**说好几道就必须输出几道，id 从 t_ai_221 起连续编号，不得缺号；禁止回卷到已占用编号**）
